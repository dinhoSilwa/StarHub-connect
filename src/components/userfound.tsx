...
export const UserFound = () => {
  const { isloading, erroFound, userFound, deleteUserFound, addStack } = foundUserStore()
  ...
  return (
    <>
      ...
      {
        userFound.map(({ id, avatar_url, name, login, public_repos, followers, stack, totalStart }, index) => (
          <section className="rounded-lg bg-white px-6 w-[90%] ml-auto mr-auto">
            ...
            <section key={index} className="flex justify-between items-center ml-auto mr-auto py-8 rounded-lg font-Montserrat shadow-sm border-zinc-200">
              ...
              {
                openStackModal ? null : <header className="flex flex-col gap-2">
